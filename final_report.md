...
```mermaid
graph TD
    subgraph ScreenGrid["Ψηφιακή Οθόνη (Display Pixel Grid)"]
        A["Εκπομπή Φωτός από Φυσικό Πλέγμα RGB Pixels<br>(Χωρική Συχνότητα f_screen)"]
    end

    subgraph SensorGrid["Αισθητήρας Κάμερας (CMOS Bayer Sensor Grid)"]
        B["Δειγματοληψία Φωτός από Πλέγμα Φίλτρου Bayer<br>(Χωρική Συχνότητα f_sensor)"]
    end

    A --> C["Αλληλεπίδραση & Παρεμβολή Συχνοτήτων"]
    B --> C

    C --> D{"f_screen ≈ f_sensor?"}
    
    D -- "Ναι (Collision)" --> E["Παραβίαση Θεωρήματος Nyquist (Aliasing)"]
    D -- "Όχι" --> F["Καθαρή Ψηφιακή Αναπαράσταση"]

    E --> G["Δημιουργία Χαμηλόσυχνης Συμβολής (Low-Frequency Spatial Beats)"]
    G --> H["Κυματοειδείς Γραμμές Παρεμβολής (Moire Scanlines)"]
    
    H --> I["Αλλοίωση Λεπτών Ακμών Χαρακτήρων (Character Outline Distortion)"]
    I --> J["Αποτυχία On-Device Latin OCR (Optical Misrecognition)"]
```
